...
export class AuthCircuitBreaker extends EventEmitter {
     ...
     */
    recordFailure(accountName, error) {
        const metrics = this.getMetrics(accountName);
        const now = Date.now();
    ...
    transitionToClosed(accountName) {
        const metrics = this.getMetrics(accountName);
        metrics.state = CircuitState.CLOSED;
        metrics.failureCount = 0;
        metrics.successCount = 0;
    ...
    transitionToHalfOpen(accountName) {
        const metrics = this.getMetrics(accountName);
    ...
    calculateNextRetryTime(metrics) {
        // Exponential backoff with jitter
        const baseDelay = Math.min(this.config.recoveryTimeoutMs * Math.pow(this.config.backoffMultiplier, metrics.failureCount - this.config.failureThreshold), this.config.maxBackoffMs);
        // Add jitter (±20%)
        const jitter = baseDelay * 0.2 * (Math.random() - 0.5);
        const delay = Math.max(1000, baseDelay + jitter); // Minimum 1 second
        metrics.nextRetryTime = Date.now() + delay;
    }
    clearOldFailures(metrics, now) {
    ...
    cleanupOldMetrics() {
        ...
        for (const [accountName, metrics] of this.metrics) {
            const lastActivity = Math.max(metrics.lastFailureTime || 0, metrics.lastSuccessTime || 0);
